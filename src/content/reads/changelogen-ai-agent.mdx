...
Since it's made with and for TypeScript, I've been using [**unjs/changelogen**](https://github.com/unjs/changelogen) to generate my `CHANGELOG.md` files. With my beloved coding agent of choice (as of this week it's Codex CLI 😜) always at hand, I created a simple **slash command** to automate the generation and most importantly to also **clean up** the output.
...
Custom slash commands (available in tools like [Codex CLI](https://developers.openai.com/codex/), Claude Code, and other AI-powered coding assistants) allow you to encapsulate repetitive workflows into reusable prompts. You create a Markdown file with instructions, and the tool turns it into a command you can invoke with `/prompts:<name>`.
...
<Note>
  **☝️ Good To Know:** Over-committed? 😜 Let the AI handle merging or
