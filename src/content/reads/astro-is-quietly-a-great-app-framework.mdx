---
title: 'Astro Is Quietly a Great App Framework'
pubDate: 2026-04-08
...
image:
  {
    src: '../../assets/reads/astro-is-quietly-a-great-app-framework.jpg',
...
import coverImage from '../../assets/reads/astro-is-quietly-a-great-app-framework.jpg'

For the last year or so, every time I sat down to start a new app, I ended up reaching for **[Astro](https://astro.build/)**. Not because I was building a blog. Not because the project needed perfect Lighthouse scores. But because Astro turned out to be the most _coherent_ way I know to ship a small-to-mid-sized full-stack app in 2026.

That's a little funny, because Astro doesn't really sell itself that way. Open the homepage and you'll see talk of **content-driven websites**, marketing pages, documentation, and blogs. All of which is true — Astro is genuinely great at all of those.
...
- A real long-running Node process you can drop in any container.
...
For race-sensitive writes — think redeem-a-code or decrement-a-counter — I lean on Drizzle's transaction API and let the database do the locking. No queue, no Redis dance, just a plain old SQL transaction.
...
Email-based flows (verification, password reset, organisation invitations) wire straight into a transactional sender — I use **[nodemailer](https://nodemailer.com/)** with whatever SMTP provider is convenient. Password hashing is done via **[`Bun.password`](https://bun.sh/docs/runtime/hashing)** — argon2id, built into the runtime, no separate `argon2` or `bcrypt` package needed. Better Auth is flexible enough to wire that in cleanly; I [walked through that exact setup](/reads/vite-nitro-minimal-fullstack) in an earlier post if you want to see it spelled out.
...
The same Zod schema that validates a request body on the server can also validate a form on the client. That's not a small thing — it means there can be exactly one source of truth for what a valid `Note`, `Invitation`, or `User` looks like.
