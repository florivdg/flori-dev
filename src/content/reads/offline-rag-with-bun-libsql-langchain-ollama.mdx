...
In order to allow follow-up questions, we might need to re-phrase the current user's question to include the context of the previous conversation. Image the following conversation:
...
You can find the full code on my [GitHub](https://github.com/florivdg/bun-docs-chat).

## Where To Go From Here?

Since this is all powered by Bun, you could easily turn this into a web-based chatbot. But for running Llama performantly you would need a quite powerful server.

Offloading the AI part to the client would be a viable option. But then all your users must run Ollama locally. You could also mess around with WebGPU and run the Llama model in the browser. There are [demos out there](https://huggingface.co/spaces/cfahlgren1/webllm-playground) that show it's possible. But that's a whole different story.

For me, the best option to grow this into something more useful would be to create a desktop app with Tauri. This way you could bundle everything together and have a nice, offline-first chatbot that you can use on your computer with a nice UI.

## Conclusion

Building an offline RAG chatbot with Bun, libSQL, LangChain.js, and Ollama was a fun and challenging project. It's great to be able to run the complete retrieval chain locally on your own computer. No need to rely on cloud services anymore. And with Llama 3.2, the quality of the generated responses is actually quite good.