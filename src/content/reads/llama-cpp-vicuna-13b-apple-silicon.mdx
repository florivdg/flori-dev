...
pubDate: 2023-07-10
description: 'Run Vicuna-13B with GPU acceleration on a Apple Silicon machine by compiling the `llama.cpp` port specifically for Apple Silicon, find a GGMLv3 compatible version of Vicuna, and execute the model with parameter `-ngl 1`.
'
image:
  {
    src: '/reads/vicuna_in_a_tech_environment_macos_terminal_gpu_acceleration.jpg',
...
First, we need to compile the `llama.cpp` port for Apple Silicon, since the official releases and Docker images don't support the M1/M2 procesors. The following command can be found in the [docs](https://github.com/ggerganov/llama.cpp#metal-build):
...
_Note:_ I used `cmake` for this step, which can be installed using [Homebrew](https://brew.sh/):
...
`llama.cpp` needs to have GGMLv3 compatible versions of the model you want it to run. So dug myself into the Hugging Face model repository and found the following versions of Vicuna: https://huggingface.co/TheBloke/stable-vicuna-13B-GGML
...
The important here is the `-ngl 1` parameter, which tells the model to use the GPU.

And et voilà, we have a working Vicuna-13B model running on an Apple Silicon GPU to chat with!
