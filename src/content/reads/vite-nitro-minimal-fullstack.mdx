...
pubDate: 2025-11-16
description: 'How I wired up Nitro 3 as a Vite plugin with Better Auth and Drizzle ORM to build a full‑stack Vue 3 app running on Bun, without giving up the simplicity of a classic Vite SPA.'
image: {
...
[Drizzle Kit](https://orm.drizzle.team/kit) is configured separately via `drizzle.config.ts` to generate migrations based on the schema under `api/_lib/schema/`.
...
const defaultDatabasePath = './.data/dimetime.sqlite'
const databasePath = process.env.DB_FILE_NAME ?? defaultDatabasePath
...
Better Auth exposes a standard **Web handler**. Nitro expects **[H3](https://h3.nuxtjs.org/) handlers**. The bridge between those two lives in `api/auth/[...all].ts`:
